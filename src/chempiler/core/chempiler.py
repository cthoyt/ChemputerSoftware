...
from chempiler.tools.chempiler_setup import Setup
from chempiler.tools.cmd_execution import Executioner
from chempiler.tools.parsing.ChASM_parser import parser

HERE = os.path.abspath(os.path.dirname(os.path.realpath(__file__)))
CRASH_DUMP_DIR = os.path.normpath(os.path.join(HERE, os.pardir, os.pardir, "client", "crash_dump"))
os.makedirs(CRASH_DUMP_DIR, exist_ok=True)
CRASH_DUMP = os.path.join(CRASH_DUMP_DIR, "crash_dump.json")
...
class Chempiler(object):
    ...
    def __init__(self, graphml_file, command_file, crash_dump=False, simulation=False):
        ...
        self.crash_dump_path = CRASH_DUMP
        if crash_dump:
            self.rebuild_graph()
    ...
    def crash_dump_json(self, data):
        ...
        """
        with open(self.crash_dump_path, "w+") as f:
    ...
    def rebuild_graph(self):
        ...
        """
        with open(self.crash_dump_path) as f:
